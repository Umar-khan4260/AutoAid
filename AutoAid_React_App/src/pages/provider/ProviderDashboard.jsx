    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-400">Welcome back, Ali Mechanic</p>
        </div>
        
        <div className="flex items-center gap-4 bg-surface-dark p-3 rounded-xl border border-gray-700">
          <span className={`text-sm font-medium ${isAvailable ? 'text-green-400' : 'text-gray-400'}`}>
            {isAvailable ? 'You are Online' : 'You are Offline'}
          </span>
          <button 
            onClick={() => setIsAvailable(!isAvailable)}
            className={`text-3xl transition-colors duration-300 ${isAvailable ? 'text-green-500' : 'text-gray-500'}`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {stats.map((stat, index) => (
          <div key={index} className="bg-surface-dark p-6 rounded-xl border border-gray-700 shadow-lg hover:border-primary/50 transition-colors duration-300">
      ...
      <div className="bg-surface-dark rounded-xl border border-gray-700 overflow-hidden">
        ...
        <div className="divide-y divide-gray-700">
          {recentActivity.map((activity) => (
            <div key={activity.id} className="p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:bg-gray-800/50 transition-colors">
              <div>
